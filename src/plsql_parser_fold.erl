...
-export([fold/5, get_stmnt_clause_curr/1, top_down/3]).
...
top_down(Module, PLSQLParseTree, Params) ->
  ?D("Start~n Module: ~p~n SQL: ~p~n Params: ~p~n", [Module, PLSQLParseTree, Params]),
...
% common processing.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-spec fold_state_common(Module :: atom(), PLSQLParseTree :: list(), Params :: any()) ->
  binary() | tuple().
fold_state_common(Module, PLSQLParseTree, Params) ->
  ParseTree =
    case PLSQLParseTree of
      [PT | _] when is_map(PT) -> lists:flatten(PLSQLParseTree, []);

      _ ->
        {ok, PT} = plsql_parser:parsetree(PLSQLParseTree),
        lists:flatten(PT, [])
    end,
...
fold(Fun, LOpts, FunState, PTree, CtxIn) ->
  ?D("Start~n LOpts: ~p~n FunState: ~p~n PTree: ~p~n CtxIn: ~p~n", [LOpts, FunState, PTree, CtxIn]),
...
fold_i(Fun, LOpts, FunState, Ctx, #{accessibleByClause := Value} = PTree) ->
  ...
  NewCtx1 =
    list_elem_ext_rule(
      ...
      NewCtxS,
      accessorCommaList,
      maps:get(accessorCommaList@, Value)
    ),
...
fold_i(Fun, LOpts, FunState, Ctx, #{assocArrayTypeDef := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(dataTypeTable@, Value)),
  NewCtx2 =
    case maps:is_key(notNull@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(notNull@, Value));
      _ -> NewCtx1
    end,
  NewCtx3 =
    case maps:is_key(dataTypeIndex@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx2, maps:get(dataTypeIndex@, Value));
      _ -> NewCtx2
    end,
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunStateIn, Ctx, #{collectionTypeDefinition := Value} = PTree) ->
  ...
  NewCtx2 =
    case maps:is_key(assocArrayTypeDef@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(assocArrayTypeDef@, Value));
      _ -> fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(varrayTypeDef@, Value))
    end,
...
% columnRef
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, #{columnRef := Value} = PTree) when is_list(Value) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = columnRef,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtxE = Fun(LOpts, FunState, NewCtxS, PTree, {Rule, 'end'}),
...
% columnRefCommaList@
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, {columnRefCommaList@, PTree}) when is_list(PTree) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = columnRefCommaList@,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = list_elem_ext_rule(Fun, LOpts, FunState, NewCtxS, columnRefCommaList, PTree),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunStateIn, Ctx, #{constantDeclaration := Value} = PTree) ->
  ...
  NewCtx3 =
    case maps:is_key(notNull@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx2, maps:get(notNull@, Value));
      _ -> NewCtx2
    end,
...
fold_i(Fun, LOpts, FunState, Ctx, #{createPackage := Value} = PTree) ->
  ...
  Rule = createPackage,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(plsqlPackageSource@, Value)),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% dataSourceCommaList@
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, {dataSourceCommaList@, PTree}) when is_list(PTree) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = dataSourceCommaList@,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = list_elem_ext_rule(Fun, LOpts, FunState, NewCtxS, dataSourceCommaList, PTree),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{dataTypeIndex := _Value} = PTree) ->
  ...
  Rule = dataTypeIndex,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(dataType@, PTree)),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% default
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, #{default := Value} = PTree) when is_map(Value) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = default,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, #{defaultValue@_@ => maps:get(value@, Value)}),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{expression := Value} = PTree) when map_size(Value) == 1 ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = expression,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, Value),
...
fold_i(Fun, LOpts, FunState, Ctx, #{expression := Value} = PTree) when map_size(Value) == 2 ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = expression,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  Operator = maps:get(operator@, Value),
  NewCtx1 =
    case is_map(Operator) of
      true -> fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(operator@, Value));
      _ -> NewCtxS
    end,
  NewCtx2 = fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(expression@, Value)),
  NewCtxE = Fun(LOpts, FunState, NewCtx2, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);

fold_i(Fun, LOpts, FunState, Ctx, #{expression := Value} = PTree) when is_map(Value) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = expression,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(expressionLeft@, Value)),
  NewCtx2 = fold_i(Fun, LOpts, FunState, NewCtx1, {operator, maps:get(operator@, Value)}),
  NewCtx3 = fold_i(Fun, LOpts, FunState, NewCtx2, maps:get(expressionRight@, Value)),
  NewCtxE = Fun(LOpts, FunState, NewCtx3, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{fieldDefinition := Value} = PTree) ->
  ...
  NewCtx3 =
    case maps:is_key(default@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx2, maps:get(default@, Value));
      _ -> NewCtx2
    end,
  NewCtxE = Fun(LOpts, FunState, NewCtx3, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{functionAnnotation := Value} = PTree) ->
  ...
  NewCtx1 =
    fold_i(
      ...
      FunState,
      NewCtxS,
      {privilegeAnnotationList@_@, maps:get(privilegeAnnotationList@, Value)}
    ),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% functionArg
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, #{functionArg := Value} = PTree) when map_size(Value) == 1 ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = functionArg,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, Value),
...
fold_i(Fun, LOpts, FunState, Ctx, #{functionArg := Value} = PTree) when map_size(Value) == 2 ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = functionArg,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(expression@, Value)),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, {functionArgCommaList@, PTree}) when is_atom(PTree) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = functionArgCommaList@,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtxE = Fun(LOpts, FunState, NewCtxS, PTree, {Rule, 'end'}),
...
fold_i(Fun, LOpts, FunState, Ctx, {functionArgCommaList@, PTree}) when is_list(PTree) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = functionArgCommaList@,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = list_elem_ext_rule(Fun, LOpts, FunState, NewCtxS, functionArgCommaList, PTree),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{functionHeading := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(parameterDeclarationCommaList@, Value) of
      true ->
        fold_i(
          Fun,
          LOpts,
          FunState,
          NewCtxS,
          {parameterDeclarationCommaList@, maps:get(parameterDeclarationCommaList@, Value)}
        );

      _ -> NewCtxS
    end,
  NewCtx2 = fold_i(Fun, LOpts, FunState, NewCtx1, {return, maps:get(return@, Value)}),
  NewCtxE = Fun(LOpts, FunState, NewCtx2, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunStateIn, Ctx, #{packageFunctionDeclaration := Value} = PTree) ->
  Rule = packageFunctionDeclaration,
  FunState = ?FOLD_INIT_STMNT(FunStateIn, Ctx, PTree, Rule),
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(apiHiddenAnnotation@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(apiHiddenAnnotation@, Value));
      _ -> NewCtxS
    end,
  NewCtx2 =
    case maps:is_key(functionAnnotation@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(functionAnnotation@, Value));
      _ -> NewCtx1
    end,
  NewCtx3 = fold_i(Fun, LOpts, FunState, NewCtx2, maps:get(functionHeading@, Value)),
  NewCtx4 =
    case maps:is_key(packageFunctionDeclarationAttributeList@, Value) of
      true ->
        list_elem_ext_rule(
          ...
          maps:get(packageFunctionDeclarationAttributeList@, Value)
        );
...
fold_i(Fun, LOpts, FunState, Ctx, #{packageItemConditional := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(expression@, Value) of
      true ->
        fold_i(Fun, LOpts, FunState, NewCtxS, #{thenExpression@_@ => maps:get(expression@, Value)});

      _ -> NewCtxS
    end,
  NewCtx2 = fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(packageItem@, Value)),
  NewCtxE = Fun(LOpts, FunState, NewCtx2, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{packageItemList@_@ := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(packageItemList@, Value) of
      true ->
        list_elem_ext_rule(
          Fun,
          LOpts,
          ...
          NewCtxS,
          packageItemList,
          maps:get(packageItemList@, Value)
        );
...
fold_i(Fun, LOpts, FunState, Ctx, #{packageItemSimple := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(packageItem@, Value)),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunStateIn, Ctx, #{packageProcedureDeclaration := Value} = PTree) ->
  ...
  NewCtx2 =
    case maps:is_key(procedureAnnotation@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(procedureAnnotation@, Value));
      _ -> NewCtx1
    end,
  NewCtx3 = fold_i(Fun, LOpts, FunState, NewCtx2, maps:get(procedureHeading@, Value)),
  NewCtx4 =
    case maps:is_key(accessibleByClause@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx3, maps:get(accessibleByClause@, Value));
      _ -> NewCtx3
    end,
  NewCtxE = Fun(LOpts, FunState, NewCtx4, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{parallelEnabledClause := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(columnRefCommaList@, Value) of
      true ->
        fold_i(
          Fun,
          LOpts,
  ...
  NewCtx2 =
    case maps:is_key(streamingClause@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(streamingClause@, Value));
      _ -> NewCtx1
    end,
  NewCtxE = Fun(LOpts, FunState, NewCtx2, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{parameterDeclaration := Value} = PTree) ->
  ...
  NewCtx2 =
    case maps:is_key(default@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(default@, Value));
      _ -> NewCtx1
    end,
  NewCtxE = Fun(LOpts, FunState, NewCtx2, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, {parameterDeclarationCommaList@, PTree}) when is_list(PTree) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = parameterDeclarationCommaList@,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = list_elem_ext_rule(Fun, LOpts, FunState, NewCtxS, parameterDeclarationCommaList, PTree),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{plsqlPackageSource := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(sharingClause@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(sharingClause@, Value));
      _ -> NewCtxS
    end,
  NewCtx2 =
    case maps:is_key(plsqlPackageSourceAttributeList@, Value) of
      true ->
        list_elem_ext_rule(
          Fun,
          LOpts,
  ...
  NewCtx3 =
    fold_i(
      ...
      FunState,
      NewCtx2,
      case {maps:is_key(man_page@, Value), maps:is_key(packageItemList@, Value)} of
        {true, true} ->
          #{
            packageItemList@_@
            =>
        ...
        {true, false} ->
          #{
            packageItemList@_@
            =>
            #{asIs@ => maps:get(asIs@, Value), man_page@ => maps:get(man_page@, Value)}
          };
        ...
        {false, false} -> #{packageItemList@_@ => #{asIs@ => maps:get(asIs@, Value)}}
      end
    ),
  NewCtxE = Fun(LOpts, FunState, NewCtx3, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, [#{plsqlUnit := Value} = PTree]) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(sqlplusCommand@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(sqlplusCommand@, Value));
      _ -> NewCtxS
    end,
  NewCtx2 = fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(createPackage@, Value)),
  NewCtxE = Fun(LOpts, FunState, NewCtx2, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunStateIn, Ctx, #{pragmaDeclaration := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(pragmaParameter@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(pragmaParameter@, Value));
      _ -> NewCtxS
    end,
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunState, Ctx, #{pragmaParameter := Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 =
    case maps:is_key(restrictReferencesList@, Value) of
      true ->
        fold_i(
          Fun,
          LOpts,
          FunState,
          NewCtxS,
          {restrictReferencesList@_@, maps:get(restrictReferencesList@, Value)}
        );
...
fold_i(Fun, LOpts, FunState, Ctx, {privilegeAnnotationList@_@, Value} = PTree) ->
  ...
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = list_elem_ext_rule(Fun, LOpts, FunState, NewCtxS, privilegeAnnotationList, Value),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
fold_i(Fun, LOpts, FunStateIn, Ctx, #{recordTypeDefinition := Value} = PTree) ->
  ...
  FunState = ?FOLD_INIT_STMNT(FunStateIn, Ctx, PTree, Rule),
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(recordTypeName@, Value)),
  NewCtx2 =
    list_elem_ext_rule(
      Fun,
      LOpts,
      ...
      maps:get(fieldDefinitionCommaList@, Value)
    ),
  NewCtxE = Fun(LOpts, FunState, NewCtx2, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, {restrictReferencesList@_@, Value} = PTree) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = restrictReferencesList@_@,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = list_elem_ext_rule(Fun, LOpts, FunState, NewCtxS, restrictReferencesList, Value),
  NewCtxE = Fun(LOpts, FunState, NewCtx1, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
% return
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold_i(Fun, LOpts, FunState, Ctx, {return, Value} = PTree) when is_map(Value) ->
  ?FOLD_INIT(FunState, Ctx, PTree),
  Rule = return,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, Value),
...
fold_i(Fun, LOpts, FunStateIn, Ctx, #{subtypeDefinition := Value} = PTree) ->
  ...
  FunState = ?FOLD_INIT_STMNT(FunStateIn, Ctx, PTree, Rule),
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(subtypeName@, Value)),
  NewCtx2 = fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(dataType@, Value)),
  NewCtx3 =
    case maps:is_key(notNull@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx2, maps:get(notNull@, Value));
...
fold_i(Fun, LOpts, FunState, Ctx, #{varrayTypeDef := Value} = PTree) ->
  ...
  Rule = varrayTypeDef,
  NewCtxS = Fun(LOpts, FunState, Ctx, PTree, {Rule, start}),
  NewCtx1 = fold_i(Fun, LOpts, FunState, NewCtxS, maps:get(type@, Value)),
  NewCtx2 = fold_i(Fun, LOpts, FunState, NewCtx1, maps:get(size@, Value)),
  NewCtx3 = fold_i(Fun, LOpts, FunState, NewCtx2, maps:get(dataType@, Value)),
  NewCtx4 =
    case maps:is_key(notNull@, Value) of
      true -> fold_i(Fun, LOpts, FunState, NewCtx3, maps:get(notNull@, Value));
      _ -> NewCtx3
    end,
  NewCtxE = Fun(LOpts, FunState, NewCtx4, PTree, {Rule, 'end'}),
  ?FOLD_RESULT(NewCtxE);
...
list_elem_ext_rule(Fun, LOpts, FunState, Ctx, Rule, Elements) ->
  ...
  case Length of
    0 -> Ctx;
    1 -> fold_i(Fun, LOpts, FunState, Ctx, {Rule, last, lists:last(Elements)});
    _ -> list_elem_ext_rule(Fun, LOpts, FunState, Ctx, Rule, Elements, Length, Length)
  end.
...
when Counter == Length ->
  ...
  list_elem_ext_rule(Fun, LOpts, FunState, NewCtxS, Rule, Tail, Counter - 1, Length);
