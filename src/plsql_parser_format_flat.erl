...
% apiGroupAnnotation
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{apiGroupAnnotation := PTree}, {apiGroupAnnotation, Step} = _FoldState) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start ->
        lists:append(
          [Ctx, "--<> ", maps:get(type@, PTree), " = ", maps:get(apiGroup@, PTree), ?CHAR_NEWLINE]
        );
...
% apiHiddenAnnotation
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{apiHiddenAnnotation := PTree}, {apiHiddenAnnotation, Step} = _FoldState) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start ->
        lists:append(
          [Ctx, "--<> ", maps:get(type@, PTree), " = ", maps:get(apiHidden@, PTree), ?CHAR_NEWLINE]
        );
...
fold(
  ...
  _FunState,
  Ctx,
  #{collectionTypeDefinition := _PTree},
...
% columnRef
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{columnRef := PTree}, {columnRef, Step} = _FoldState) when is_list(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> Ctx ++ PTree;
...
% constantDeclaration
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{constantDeclaration := _PTree}, {constantDeclaration, Step} = _FoldState) ->
  ?CUSTOM_INIT(_FunState, Ctx, _PTree, _FoldState),
  RT =
    case Step of
      start -> Ctx;
...
fold([], _FunState, Ctx, #{constantName := PTree}, {constantName, Step} = _FoldState) ->
  ...
  RT =
    case Step of
      start -> lists:append([Ctx, PTree, " constant"]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
% dataType
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{dataType := PTree}, {dataType, Step} = _FoldState) when is_map(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
...
% default
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{default := PTree}, {default, Step} = _FoldState) when is_map(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> lists:append([Ctx, " ", maps:get(type@, PTree)]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
fold(
  ...
  RT =
    case Step of
      start -> Ctx;
      _ -> lists:append([Ctx, PTree, " exception;"])
    end,
  ?CUSTOM_RESULT(RT);
...
fold([], _FunState, Ctx, #{expression := PTree}, {expression, Step} = _FoldState) when is_map(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  Operator =
...
% fieldDefinition
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{fieldDefinition := PTree}, {fieldDefinition, Step} = _FoldState)
when is_map(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
...
when is_map(PTree) ->
  ...
  RT =
    case Step of
      start -> lists:append([Ctx, "function ", maps:get(name@, PTree)]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
when is_list(PTree) ->
  ...
  RT =
    case Step of
      start -> lists:append([Ctx, " authid ", PTree]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
% literal
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{literal := PTree}, {literal, Step} = _FoldState) when is_list(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> lists:append([Ctx, PTree]);
...
% operator
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, PTree, {operator, Step} = _FoldState) when is_atom(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> lists:append([Ctx, " ", atom_to_list(PTree), " "]);
...
fold(
  ...
  _FunState,
  Ctx,
  #{packageFunctionDeclarationAttribute := PTree},
...
when is_map(PTree) ->
  ...
  RT =
    case Step of
      start -> lists:append([Ctx, maps:get(start@, PTree), " "]);
        ...
        lists:append(
          [
            ...
            case maps:is_key(implementationPackage@, PTree) of
              true -> " using " ++ maps:get(implementationPackage@, PTree);
              _ -> []
            end
          ]
        );
...
fold([], _FunState, Ctx, #{plsqlPackageSource := PTree}, {plsqlPackageSource, Step} = _FoldState) ->
  ...
  RT =
    case Step of
      start -> lists:append([Ctx, " ", maps:get(packageNameStart@, PTree)]);
...
% pragmaDeclaration
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{pragmaDeclaration := PTree}, {pragmaDeclaration, Step} = _FoldState) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> lists:append([Ctx, "pragma ", maps:get(pragmaType@, PTree)]);
      _ -> Ctx ++ ";"
    end,
  ?CUSTOM_RESULT(RT);
...
% pragmaParameter
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{pragmaParameter := PTree}, {pragmaParameter, Step} = _FoldState)
when is_map(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
        ...
        lists:append(
          [
            ...
            case maps:is_key(sign@, PTree) of
              true -> lists:append([maps:get(sign@, PTree)]);
              _ -> []
            end,
            case maps:is_key(value@, PTree) of
              true -> lists:append([maps:get(value@, PTree)]);
              _ -> []
            end
          ]
        );
...
when is_map(PTree) ->
  ...
  RT =
    case Step of
      start -> lists:append([Ctx, "procedure ", maps:get(name@, PTree)]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
% recordTypeName
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{recordTypeName := PTree}, {recordTypeName, Step} = _FoldState) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> lists:append([Ctx, "type ", PTree, " is record ("]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
% restrictReferences
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{restrictReferences := PTree}, {restrictReferences, Step} = _FoldState)
when is_list(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
...
when is_list(PTree) ->
  ...
  RT =
    case Step of
      start -> lists:append([Ctx, " sharing = ", PTree]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
% sqlplusCommand
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{sqlplusCommand := PTree}, {sqlplusCommand, Step} = _FoldState)
when is_list(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
...
% subtypeDefinition
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{subtypeDefinition := _PTree}, {subtypeDefinition, Step} = _FoldState) ->
  ?CUSTOM_INIT(_FunState, Ctx, _PTree, _FoldState),
  RT =
    case Step of
      start -> Ctx;
...
% subtypeName
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{subtypeName := PTree}, {subtypeName, Step} = _FoldState) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> lists:append([Ctx, "subtype ", PTree, " is"]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
fold([], _FunState, Ctx, #{typeName := PTree}, {typeName, Step} = _FoldState) ->
  ...
  RT =
    case Step of
      start -> lists:append([Ctx, "type ", PTree, " is "]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
% variableDeclaration
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{variableDeclaration := PTree}, {variableDeclaration, Step} = _FoldState) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> Ctx ++ maps:get(name@, PTree);
...
% varraySize
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{varraySize := PTree}, {varraySize, Step} = _FoldState)
when is_list(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
      start -> lists:append([Ctx, "(", PTree, ") of"]);
      _ -> Ctx
    end,
  ?CUSTOM_RESULT(RT);
...
% varrayType
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold([], _FunState, Ctx, #{varrayType := PTree}, {varrayType, Step} = _FoldState)
when is_list(PTree) ->
  ?CUSTOM_INIT(_FunState, Ctx, PTree, _FoldState),
  RT =
    case Step of
...
when Rule == packageFunctionDeclarationAttributeList;
