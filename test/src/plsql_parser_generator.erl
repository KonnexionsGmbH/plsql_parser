      ...
      lists:append(
        [
          case rand:uniform(2) rem 2 of
            1 -> lists:nth(rand:uniform(UnitKind_Length), UnitKind) ++ " ";
            _ -> []
          end,
          case rand:uniform(2) rem 2 of
...
create_code(accessibleByClause = Rule) ->
  ?CREATE_CODE_START,
  [{accessorCommaList, AccessorCommaList}] = ets:lookup(?CODE_TEMPLATES, accessorCommaList),
...
create_code(apiGroupAnnotation = Rule) ->
  ...
  Code =
    [
      lists:append(["--<> API_Group = ", lists:nth(rand:uniform(Name_Length), Name), ?CHAR_NEWLINE])
      || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(apiHiddenAnnotation = Rule) ->
  ?CREATE_CODE_START,
  Code = ["--<> API_Hidden = True" ++ ?CHAR_NEWLINE],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
      ...
      lists:append(
        [
          ...
          case rand:uniform(5) rem 5 of
            1 -> lists:nth(rand:uniform(DataType_2_Length), DataType_2);
            _ -> lists:nth(rand:uniform(DataType_1_Length), DataType_1)
          end,
          case rand:uniform(5) rem 5 of
            1 -> " not null ";
            _ -> []
          ...
          case rand:uniform(2) rem 2 of
            1 ->
              " index by " ++ case rand:uniform(10) rem 10 of
                1 -> lists:nth(rand:uniform(DataType_3_Length), DataType_3);
                _ -> lists:nth(rand:uniform(DataType_2_Length), DataType_2)
              end;
...
create_code(collectionTypeDefinition = Rule) ->
  ?CREATE_CODE_START,
  [{assocArrayTypeDef, AssocArrayTypeDef}] = ets:lookup(?CODE_TEMPLATES, assocArrayTypeDef),
  AssocArrayTypeDef_Length = length(AssocArrayTypeDef),
  [{name, Name}] = ets:lookup(?CODE_TEMPLATES, name),
  Name_Length = length(Name),
  [{varrayTypeDef, VarrayTypeDef}] = ets:lookup(?CODE_TEMPLATES, varrayTypeDef),
    ...
    [
      lists:append(
        [
          ...
          " is ",
          case rand:uniform(2) rem 2 of
            1 -> lists:nth(rand:uniform(AssocArrayTypeDef_Length), AssocArrayTypeDef);
            _ -> lists:nth(rand:uniform(VarrayTypeDef_Length), VarrayTypeDef)
          end,
          ";"
        ]
      ) || _ <- lists:seq(1, ?MAX_BASIC * 2)
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        ...
        _ ->
          lists:nth(rand:uniform(Name_Length), Name) ++ case rand:uniform(3) rem 3 of
            1 -> " (+)";
            2 -> ".*";
            _ -> []
          end
...
create_code(createPackage = Rule) ->
  ?CREATE_CODE_START,
  [{plsqlPackageSource, PlsqlPackageSource}] = ets:lookup(?CODE_TEMPLATES, plsqlPackageSource),
  PlsqlPackageSource_Length = length(PlsqlPackageSource),
  [{sqlplusCommand, SqlplusCommand}] = ets:lookup(?CODE_TEMPLATES, sqlplusCommand),
  ...
  Code =
    [
      lists:append(
        [
          case rand:uniform(5) rem 5 of
            1 -> lists:nth(rand:uniform(SqlplusCommand_Length), SqlplusCommand);
            _ -> []
          end,
          "Create",
          case rand:uniform(2) rem 2 of
          ...
          " Package ",
          lists:nth(rand:uniform(PlsqlPackageSource_Length), PlsqlPackageSource),
          ";",
          case rand:uniform(2) rem 2 of
            1 -> "/";
            _ -> []
...
create_code(dataType_1 = Rule) ->
  ...
  Code =
    [
      ...
      "Boolean",
      "Char",
      lists:append(["Char(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      lists:append(["Char(", lists:nth(rand:uniform(Intnum_Length), Intnum), " Byte)"]),
      lists:append(["Char(", lists:nth(rand:uniform(Intnum_Length), Intnum), " Char)"]),
      "Clob",
      "Date",
      "Float",
      lists:append(["Float(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      "Interval Day To Second",
      lists:append(["Interval Day To Second(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      lists:append(
        [
          "Interval Day(",
      ...
      lists:append(
        ["Interval Year(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")", "To Month"]
      ),
      "Long Raw",
      lists:nth(rand:uniform(Name_Length), Name),
      lists:append(
      ...
      lists:append(["Nchar(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      "Nclob",
      "Number",
      lists:append(["Number(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      lists:append(
        [
          "Number(",
          lists:nth(rand:uniform(Intnum_Length), Intnum),
      ...
      lists:append(["Nvarchar2(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      lists:append(["Raw(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      "Raw",
      "Ref Cursor",
      "Rowid",
      "Timestamp",
      ...
      lists:append(["Timestamp(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      lists:append(
        ["Timestamp(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")With Time Zone"]
      ),
      lists:append(
        ["Timestamp(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")With Local Time Zone"]
      ),
      "Urowid",
      lists:append(["Urowid(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"]),
      "Varchar2",
      lists:append(["Varchar2(", lists:nth(rand:uniform(Intnum_Length), Intnum), "Byte)"]),
      lists:append(["Varchar2(", lists:nth(rand:uniform(Intnum_Length), Intnum), "Char)"]),
      "Xmltype"
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(dataType_2 = Rule) ->
  ...
  Code =
    [
      ...
      "Pls_integer",
      lists:append(["Varchar2(", lists:nth(rand:uniform(Intnum_Length), Intnum), ")"])
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        1 -> " := ";
        _ -> " Default "
      end ++ lists:nth(rand:uniform(Expression_Length), Expression) ||
      _
      <-
...
create_code(defaultCollationClause = Rule) ->
  ?CREATE_CODE_START,
  Code = ["Default Collation Using_nls_comp"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(expression = Rule) ->
  ...
  [{expression, Expression}] = ets:lookup(?CODE_TEMPLATES, expression),
  Expression_Length = length(Expression),
  [{unaryAddOrSubtract, UnaryAddOrSubtract}] = ets:lookup(?CODE_TEMPLATES, unaryAddOrSubtract),
  UnaryAddOrSubtract_Length = length(UnaryAddOrSubtract),
  Code =
    [
      case rand:uniform(10) rem 10 of
        1 -> lists:append(["(", lists:nth(rand:uniform(Expression_Length), Expression), ")"]);
        2 -> "Not " ++ lists:nth(rand:uniform(Expression_Length), Expression);

        3 ->
          lists:append(
            [
              lists:nth(rand:uniform(UnaryAddOrSubtract_Length), UnaryAddOrSubtract),
              " ",
          ...
          lists:append(
            [
              ...
              case rand:uniform(15) rem 15 of
                ...
                7 -> "=";
                8 -> "||";
                _ -> lists:nth(rand:uniform(Comparison_Length), Comparison)
              end,
              " ",
              lists:nth(rand:uniform(Expression_Length), Expression),
              ")"
      ...
      lists:append(
        [
          ...
          case rand:uniform(4) rem 4 of
            1 -> " not null " ++ lists:nth(rand:uniform(Default_Length), Default);
...
create_code(fieldDefinitionCommaList = Rule) ->
  ?CREATE_CODE_START,
  [{fieldDefinition, FieldDefinition}] = ets:lookup(?CODE_TEMPLATES, fieldDefinition),
    ...
    [
      case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
              lists:nth(rand:uniform(FieldDefinition_Length), FieldDefinition),
              ",",
              lists:nth(rand:uniform(FieldDefinition_Length), FieldDefinition)
            ]
          );

        _ -> lists:nth(rand:uniform(FieldDefinition_Length), FieldDefinition)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(functionAnnotation = Rule) ->
  ?CREATE_CODE_START,
  [{apiHiddenAnnotation, ApiHiddenAnnotation}] = ets:lookup(?CODE_TEMPLATES, apiHiddenAnnotation),
  ...
  Code =
    [
      case rand:uniform(5) rem 5 of
        1 -> lists:nth(rand:uniform(ApiHiddenAnnotation_Length), ApiHiddenAnnotation);
        _ -> []
      end ++ lists:nth(rand:uniform(PrivilegeAnnotationList_Length), PrivilegeAnnotationList) ||
      _
      <-
      lists:seq(1, ?MAX_BASIC * 2)
    ],
    ...
    [
      case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
              bracket_query_spec(lists:nth(rand:uniform(FunctionArg_Length), FunctionArg)),
              ",",
              bracket_query_spec(lists:nth(rand:uniform(FunctionArg_Length), FunctionArg))
            ]
          );

        _ -> bracket_query_spec(lists:nth(rand:uniform(FunctionArg_Length), FunctionArg))
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(functionHeading = Rule) ->
  ?CREATE_CODE_START,
  [{dataTypeReturn, DataTypeReturn}] = ets:lookup(?CODE_TEMPLATES, dataTypeReturn),
  ...
  ParameterDeclarationCommaList_Length = length(ParameterDeclarationCommaList),
  Code =
    [
      lists:append(
        [
            ...
            _ ->
              lists:append(
                [
                  "(",
                  lists:nth(
                    rand:uniform(ParameterDeclarationCommaList_Length),
        ...
        1 ->
          lists:append(
            [
              ...
              lists:nth(rand:uniform(Name_Length), Name),
              " (",
              lists:nth(rand:uniform(FunctionArgCommaList_Length), FunctionArgCommaList),
              ")"
            ]
          );
...
create_code(invokerRightsClause = Rule) ->
  ?CREATE_CODE_START,
  Code = ["Authid Current_user", "Authid Definer"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
      ...
      lists:append(
        [
          ...
          case rand:uniform(2) rem 2 of
            ...
            _ ->
              lists:append(
                ["Comment Line No. ", integer_to_list(rand:uniform(?MAX_BASIC)), ?CHAR_NEWLINE]
              )
          end,
          "*/",
          ?CHAR_NEWLINE
...
create_code(nameExtended = Rule) ->
  ?CREATE_CODE_START,
  [{name, Name}] = ets:lookup(?CODE_TEMPLATES, name),
  Code = ["api_group", "false", "none" "true"] ++ Name,
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(objectPrivilegeAnnotation = Rule) ->
  ...
  [{name, Name}] = ets:lookup(?CODE_TEMPLATES, name),
  Name_Length = length(Name),
  [{objectPrivilegeType, ObjectPrivilegeType}] = ets:lookup(?CODE_TEMPLATES, objectPrivilegeType),
  ...
  Code =
    [
      lists:append(
        [
          "--<> Object_privilege ",
          lists:nth(rand:uniform(ObjectPrivilegeType_Length), ObjectPrivilegeType),
          "=",
          case rand:uniform(2) rem 2 of
            1 ->
              lists:append(
...
create_code(packageFunctionDeclaration = Rule) ->
  ?CREATE_CODE_START,
  [{functionAnnotation, FunctionAnnotation}] = ets:lookup(?CODE_TEMPLATES, functionAnnotation),
  FunctionAnnotation_Length = length(FunctionAnnotation),
  [{functionHeading, FunctionHeading}] = ets:lookup(?CODE_TEMPLATES, functionHeading),
  ...
  [{packageFunctionDeclarationAttributeList, PackageFunctionDeclarationAttributeList}] =
    ets:lookup(?CODE_TEMPLATES, packageFunctionDeclarationAttributeList),
  PackageFunctionDeclarationAttributeList_Length = length(PackageFunctionDeclarationAttributeList),
  Code =
    [
      lists:append(
        [
          case rand:uniform(2) rem 2 of
            1 -> lists:nth(rand:uniform(FunctionAnnotation_Length), FunctionAnnotation) ++ " ";
            _ -> []
          end,
          lists:nth(rand:uniform(FunctionHeading_Length), FunctionHeading) ++ case
          rand:uniform(2) rem 2 of
            1 ->
              " "
              ++
              lists:nth(
                rand:uniform(PackageFunctionDeclarationAttributeList_Length),
...
create_code(packageFunctionDeclarationAttribute = Rule) ->
  ?CREATE_CODE_START,
  [{accessibleByClause, AccessibleByClause}] = ets:lookup(?CODE_TEMPLATES, accessibleByClause),
  AccessibleByClause_Length = length(AccessibleByClause),
  [{parallelEnabledClause, ParallelEnabledClause}] =
    ets:lookup(?CODE_TEMPLATES, parallelEnabledClause),
  ParallelEnabledClause_Length = length(ParallelEnabledClause),
  [{pipelinedClause, PipelinedClause}] = ets:lookup(?CODE_TEMPLATES, pipelinedClause),
  PipelinedClause_Length = length(PipelinedClause),
  [{resultCacheClause, ResultCacheClause}] = ets:lookup(?CODE_TEMPLATES, resultCacheClause),
  ResultCacheClause_Length = length(ResultCacheClause),
  Code =
    [
      case rand:uniform(5) rem 5 of
        1 -> lists:nth(rand:uniform(AccessibleByClause_Length), AccessibleByClause);
        2 -> lists:nth(rand:uniform(ParallelEnabledClause_Length), ParallelEnabledClause);
        3 -> lists:nth(rand:uniform(PipelinedClause_Length), PipelinedClause);
        4 -> lists:nth(rand:uniform(ResultCacheClause_Length), ResultCacheClause);
        _ -> "Deterministic"
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
...
create_code(packageFunctionDeclarationAttributeList = Rule) ->
  ...
  [{packageFunctionDeclarationAttribute, PackageFunctionDeclarationAttribute}] =
    ets:lookup(?CODE_TEMPLATES, packageFunctionDeclarationAttribute),
  PackageFunctionDeclarationAttribute_Length = length(PackageFunctionDeclarationAttribute),
  Code =
    [
      case rand:uniform(2) rem 2 of
        1 ->
...
create_code(packageItem = Rule) ->
  ?CREATE_CODE_START,
  [{collectionTypeDefinition, CollectionTypeDefinition}] =
    ets:lookup(?CODE_TEMPLATES, collectionTypeDefinition),
  CollectionTypeDefinition_Length = length(CollectionTypeDefinition),
  [{constantDeclaration, ConstantDeclaration}] = ets:lookup(?CODE_TEMPLATES, constantDeclaration),
  ConstantDeclaration_Length = length(ConstantDeclaration),
  [{exceptionDeclaration, ExceptionDeclaration}] =
    ets:lookup(?CODE_TEMPLATES, exceptionDeclaration),
  ExceptionDeclaration_Length = length(ExceptionDeclaration),
  [{packageFunctionDeclaration, PackageFunctionDeclaration}] =
    ets:lookup(?CODE_TEMPLATES, packageFunctionDeclaration),
  PackageFunctionDeclaration_Length = length(PackageFunctionDeclaration),
  [{packageProcedureDeclaration, PackageProcedureDeclaration}] =
    ets:lookup(?CODE_TEMPLATES, packageProcedureDeclaration),
  PackageProcedureDeclaration_Length = length(PackageProcedureDeclaration),
  [{recordTypeDefinition, RecordTypeDefinition}] =
    ets:lookup(?CODE_TEMPLATES, recordTypeDefinition),
  RecordTypeDefinition_Length = length(RecordTypeDefinition),
  [{refCursorTypeDefinition, RefCursorTypeDefinition}] =
    ets:lookup(?CODE_TEMPLATES, refCursorTypeDefinition),
  RefCursorTypeDefinition_Length = length(RefCursorTypeDefinition),
  [{subtypeDefinition, SubtypeDefinition}] = ets:lookup(?CODE_TEMPLATES, subtypeDefinition),
  ...
  Code =
    [
      case rand:uniform(20) rem 20 of
        1 ->
          case rand:uniform(6) rem 6 of
            1 -> lists:nth(rand:uniform(ConstantDeclaration_Length), ConstantDeclaration);
            2 -> lists:nth(rand:uniform(ExceptionDeclaration_Length), ExceptionDeclaration);
            3 -> lists:nth(rand:uniform(RecordTypeDefinition_Length), RecordTypeDefinition);
            4 -> lists:nth(rand:uniform(RefCursorTypeDefinition_Length), RefCursorTypeDefinition);

            5 ->
              case rand:uniform(4) rem 4 of
                1 -> lists:nth(rand:uniform(SubtypeDefinition_Length), SubtypeDefinition);

                _ ->
                  lists:nth(rand:uniform(CollectionTypeDefinition_Length), CollectionTypeDefinition)
              end;

            _ -> lists:nth(rand:uniform(VariableDeclaration_Length), VariableDeclaration)
          end;

        _ ->
          case rand:uniform(2) rem 2 of
            1 ->
              lists:nth(rand:uniform(PackageFunctionDeclaration_Length), PackageFunctionDeclaration);
  ...
  Code =
    [
      case rand:uniform(3) rem 3 of
        1 ->
          lists:append(
            ["$else ", lists:nth(rand:uniform(PackageItem_Length), PackageItem), " $end"]
          );
...
create_code(packageItemList = Rule) ->
  ...
  [{packageItem, PackageItem}] = ets:lookup(?CODE_TEMPLATES, packageItem),
  PackageItem_Length = length(PackageItem),
  [{packageItemConditional, PackageItemConditional}] =
    ets:lookup(?CODE_TEMPLATES, packageItemConditional),
      ...
      case rand:uniform(4) rem 4 of
        1 ->
          lists:append(
            [
              case rand:uniform(2) rem 2 of
                1 -> lists:nth(rand:uniform(PackageItem_Length), PackageItem);
                _ -> lists:nth(rand:uniform(PackageItemConditional_Length), PackageItemConditional)
              end,
              " ",
              lists:nth(rand:uniform(PackageItem_Length), PackageItem),
              " ",
...
create_code(packageProcedureDeclaration = Rule) ->
  ...
  [{accessibleByClause, AccessibleByClause}] = ets:lookup(?CODE_TEMPLATES, accessibleByClause),
  AccessibleByClause_Length = length(AccessibleByClause),
  [{procedureAnnotation, ProcedureAnnotation}] = ets:lookup(?CODE_TEMPLATES, procedureAnnotation),
  [{man_page, Man_Page}] = ets:lookup(?CODE_TEMPLATES, man_page),
  Man_Page_Length = length(Man_Page),
  ProcedureAnnotation_Length = length(ProcedureAnnotation),
  [{procedureHeading, ProcedureHeading}] = ets:lookup(?CODE_TEMPLATES, procedureHeading),
  ...
  Code =
    [
      lists:append(
        [
          case rand:uniform(2) rem 2 of
            1 -> lists:nth(rand:uniform(ProcedureAnnotation_Length), ProcedureAnnotation) ++ " ";
            _ -> []
          end,
          lists:nth(rand:uniform(ProcedureHeading_Length), ProcedureHeading),
          case rand:uniform(2) rem 2 of
            1 -> " " ++ lists:nth(rand:uniform(AccessibleByClause_Length), AccessibleByClause);
            _ -> []
          end,
...
create_code(parallelEnabledClause = Rule) ->
  ...
  [{columnRef, ColumnRef}] = ets:lookup(?CODE_TEMPLATES, columnRef),
  ColumnRef_Length = length(ColumnRef),
  [{columnRefCommaList, ColumnRefCommaList}] = ets:lookup(?CODE_TEMPLATES, columnRefCommaList),
  ColumnRefCommaList_Length = length(ColumnRefCommaList),
  [{name, Name}] = ets:lookup(?CODE_TEMPLATES, name),
  Name_Length = length(Name),
  [{streamingClause, StreamingClause}] = ets:lookup(?CODE_TEMPLATES, streamingClause),
      ...
      lists:append(
        [
          ...
          case rand:uniform(5) rem 5 of
            ...
            2 ->
              lists:append(["Value (", lists:nth(rand:uniform(ColumnRef_Length), ColumnRef), ")"]);
            ...
            _ ->
              lists:append(
                [
                  ...
                  lists:nth(rand:uniform(ColumnRefCommaList_Length), ColumnRefCommaList),
                  ")",
                  case rand:uniform(2) rem 2 of
                    1 -> lists:nth(rand:uniform(StreamingClause_Length), StreamingClause);
                    _ -> []
                  end
                ]
              )
...
create_code(parameterDeclarationCommaList = Rule) ->
  ...
  [{parameterDeclaration, ParameterDeclaration}] =
    ets:lookup(?CODE_TEMPLATES, parameterDeclaration),
    ...
    [
      case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
              lists:nth(rand:uniform(ParameterDeclaration_Length), ParameterDeclaration),
              ",",
              lists:nth(rand:uniform(ParameterDeclaration_Length), ParameterDeclaration)
            ]
          );

        _ -> lists:nth(rand:uniform(ParameterDeclaration_Length), ParameterDeclaration)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(plsqlPackageSource = Rule) ->
  ...
  [{name, Name}] = ets:lookup(?CODE_TEMPLATES, name),
  Name_Length = length(Name),
  [{sharingClause, SharingClause}] = ets:lookup(?CODE_TEMPLATES, sharingClause),
  SharingClause_Length = length(SharingClause),
  [{plsqlPackageSourceAttributeList, PlsqlPackageSourceAttributeList}] =
    ets:lookup(?CODE_TEMPLATES, plsqlPackageSourceAttributeList),
  PlsqlPackageSourceAttributeList_Length = length(PlsqlPackageSourceAttributeList),
  [{packageItemList, PackageItemList}] = ets:lookup(?CODE_TEMPLATES, packageItemList),
    ...
    [
      lists:append(
        [
          ...
          lists:nth(rand:uniform(Name_Length), Name),
          case rand:uniform(2) rem 2 of
            1 -> " " ++ lists:nth(rand:uniform(SharingClause_Length), SharingClause);
            _ -> []
          end,
          case rand:uniform(2) rem 2 of
            1 ->
              ...
              ++
              lists:nth(
                rand:uniform(PlsqlPackageSourceAttributeList_Length),
          ...
          case rand:uniform(5) rem 5 of
            1 -> lists:nth(rand:uniform(PackageItemList_Length), PackageItemList);
            _ -> []
          end,
          "End",
          case rand:uniform(2) rem 2 of
...
create_code(plsqlPackageSourceAttribute = Rule) ->
  ...
  [{accessibleByClause, AccessibleByClause}] = ets:lookup(?CODE_TEMPLATES, accessibleByClause),
  AccessibleByClause_Length = length(AccessibleByClause),
  [{defaultCollationClause, DefaultCollationClause}] =
    ets:lookup(?CODE_TEMPLATES, defaultCollationClause),
  DefaultCollationClause_Length = length(DefaultCollationClause),
  [{invokerRightsClause, InvokerRightsClause}] = ets:lookup(?CODE_TEMPLATES, invokerRightsClause),
  InvokerRightsClause_Length = length(InvokerRightsClause),
  Code =
    [
      case rand:uniform(3) rem 3 of
        1 -> lists:nth(rand:uniform(AccessibleByClause_Length), AccessibleByClause);
        2 -> lists:nth(rand:uniform(DefaultCollationClause_Length), DefaultCollationClause);
        _ -> lists:nth(rand:uniform(InvokerRightsClause_Length), InvokerRightsClause)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(plsqlPackageSourceAttributeList = Rule) ->
  ?CREATE_CODE_START,
  [{plsqlPackageSourceAttribute, PlsqlPackageSourceAttribute}] =
    ets:lookup(?CODE_TEMPLATES, plsqlPackageSourceAttribute),
  ...
  Code =
    [
      case rand:uniform(2) rem 2 of
        ...
        _ ->
          lists:nth(rand:uniform(PlsqlPackageSourceAttribute_Length), PlsqlPackageSourceAttribute)
      end || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(pragmaDeclaration = Rule) ->
  ...
  PragmaParameterRestrictReferences_Length = length(PragmaParameterRestrictReferences),
  Code =
    ["Pragma Serially_reusable", "Pragma Udf"] ++ [
      "Pragma " ++ case rand:uniform(2) rem 2 of
        1 ->
          "Exception_init"
          ++
          lists:nth(rand:uniform(PragmaParameterExceptionInit_Length), PragmaParameterExceptionInit);

        _ ->
          "Restrict_references"
          ++
          lists:nth(
            rand:uniform(PragmaParameterRestrictReferences_Length),
      ...
      lists:append(
        [
          ...
          case rand:uniform(5) rem 5 of
            ...
            _ -> "-" ++ lists:nth(rand:uniform(Intnum_Length), Intnum)
          end,
          ")"
        ]
      ) || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ...
    [
      lists:append(
        [
          ...
          ",",
          lists:nth(rand:uniform(RestrictReferencesList_Length), RestrictReferencesList),
          ")"
        ]
      ) || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ],
...
create_code(privilegeAnnotationList = Rule) ->
  ?CREATE_CODE_START,
  [{apiGroupAnnotation, ApiGroupAnnotation}] = ets:lookup(?CODE_TEMPLATES, apiGroupAnnotation),
    ...
    [
      case rand:uniform(2) rem 2 of
        1 ->
          case rand:uniform(3) rem 3 of
            1 ->
              lists:nth(rand:uniform(ObjectPrivilegeAnnotation_Length), ObjectPrivilegeAnnotation);

            2 -> lists:nth(rand:uniform(ApiGroupAnnotation_Length), ApiGroupAnnotation);

            _ ->
              lists:nth(rand:uniform(SystemPrivilegeAnnotation_Length), SystemPrivilegeAnnotation)
          end,
          case rand:uniform(3) rem 3 of
            1 ->
              lists:nth(rand:uniform(ObjectPrivilegeAnnotation_Length), ObjectPrivilegeAnnotation);
    ...
    [
      lists:append(
        [
          ...
          lists:nth(rand:uniform(Name_Length), Name),
          " is record (",
          lists:nth(rand:uniform(FieldDefinitionCommaList_Length), FieldDefinitionCommaList),
          ");"
        ]
      )
      || _ <- lists:seq(1, ?MAX_BASIC * 2)
    ...
    [
      lists:append(
        [
          ...
          case rand:uniform(6) rem 6 of
            1 -> lists:nth(rand:uniform(Name_Length), Name);
            2 -> lists:nth(rand:uniform(Name_Length), Name) ++ "%Rowtype";
            3 -> lists:nth(rand:uniform(Name_Length), Name) ++ "%Type";
...
create_code(restrictReferencesList = Rule) ->
  ?CREATE_CODE_START,
  RestrictReferences = ["Rnds", "Rnps", "Trust", "Wnds", "Wnps"],
    ...
    [
      case rand:uniform(3) rem 3 of
        1 ->
          lists:append(
            [
              lists:nth(rand:uniform(RestrictReferences_Length), RestrictReferences),
              case rand:uniform(2) rem 2 of
                1 -> ",";
                _ -> " "
              end,
        ...
        _ ->
          lists:nth(rand:uniform(RestrictReferences_Length), RestrictReferences) ++ case
          rand:uniform(2) rem 2 of
            1 -> ",";
            _ -> []
          end
...
create_code(resultCacheClause = Rule) ->
  ?CREATE_CODE_START,
  [{dataSourceCommaList, DataSourceCommaList}] = ets:lookup(?CODE_TEMPLATES, dataSourceCommaList),
  DataSourceCommaList_Length = length(DataSourceCommaList),
  Code =
    [
      "Result_cache" ++ case rand:uniform(2) rem 2 of
        1 ->
          lists:append(
            [
              " Relies_on(",
              lists:nth(rand:uniform(DataSourceCommaList_Length), DataSourceCommaList),
              ")"
            ]
          );

        _ -> []
      end || _ <- lists:seq(1, ?MAX_BASIC div 10)
    ],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(sharingClause = Rule) ->
  ?CREATE_CODE_START,
  Code = ["Sharing = Metadata", "Sharing = None"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(sqlplusCommand = Rule) ->
  ?CREATE_CODE_START,
  Code = ["Set Define Off;", "Set Define On;"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
create_code(systemPrivilegeAnnotation = Rule) ->
  ?CREATE_CODE_START,
  [{systemPrivilegeType, SystemPrivilegeType}] = ets:lookup(?CODE_TEMPLATES, systemPrivilegeType),
  ...
  Code =
    [
      lists:append(
        [
          "--<> System_privilege = ",
          lists:nth(rand:uniform(SystemPrivilegeType_Length), SystemPrivilegeType),
          ?CHAR_NEWLINE
        ]
      )
      || _ <- lists:seq(1, ?MAX_BASIC * 2)
...
create_code(unitKind = Rule) ->
  ?CREATE_CODE_START,
  Code = ["Function", "Package", "Procedure", "Trigger", "Type"],
  store_code(Rule, Code, ?MAX_BASIC, false),
  ?CREATE_CODE_END;
...
file_create_ct_all(_Type, _Complete, _CompactedDetailed, []) -> ok;
...
file_create_ct(Type, Complete, CompactedDetailed, Rule) ->
  ...
  FileName = lists:append([Type, "_", Complete, "_", CompactedDetailed, "_", RuleString, "_SUITE"]),
  ...
  case CodeLength of
    ...
    _ ->
      ...
      file_write_ct(1, Type, Complete, CompactedDetailed, File, Code)
  end.
...
file_write_ct(Current, Type, Complete, CompactedDetailed, File, [H | T]) ->
        ...
        [
          lists:append(
            [
              "    ",
              case Type of
                "performance" -> "{ok, _} = plsql_parser:parsetree_with_tokens";
  ...
  file_write_ct(Current + 1, Type, Complete, CompactedDetailed, File, T).
...
file_create_eunit_all(_Type, _Complete, []) -> ok;
...
file_create_eunit(Type, Complete, Rule) ->
  ...
  FileName = lists:append([Type, "_", Complete, "_", RuleStrimg, ".tst"]),
...
file_write_eunit(_Complete, File, []) -> file:close(File);
